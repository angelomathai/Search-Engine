...
$start="http://localhost:8888/Web_crawler/test.html";
        ...
        foreach ($linklist as $link)
        {
            ...
            if(!in_array($l,$already_crawled))
            {
                ...
                if($rows>0)
                {
                    ...
                    {
                        $result=$pdo->prepare("UPDATE `index` SET  title=:title,description=:description,keywords=:keywords,url_hash=:url,url=:url_hash WHERE url_hash=:url_hash");
                ...
                else
                {
                    ...
                    if(!is_null($params[':title']) && !is_null($params[':description']) && $params[':title']!='')
                    {
                        $result=$pdo->prepare("INSERT INTO `index` VALUES (null,:title,:description,:keywords,:url,:url_hash)");
                        $result=$result->execute($params);
                    }
                }
            }
